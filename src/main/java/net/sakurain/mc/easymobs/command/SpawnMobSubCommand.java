...
public class SpawnMobSubCommand implements SubCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String[] args) {
        if (args.length < 1) {
            sender.sendMessage(MessageUtil.error("Usage: /ezmobs spawn <mob-id> [player|x y z]"));
            return true;
        }

        String mobId = args[0];
        CustomMobManager manager = EasyMobsPlugin.getInstance().getMobManager();
